 ...
 * Needs `npm run preview` (or --url). Runs 2 missions concurrently (swiftshader
 * WebGL is CPU-rendered — higher concurrency starves page loads into timeouts).
 *
 *   node scripts/test_campaign.mjs [--only m04,m21b] [--url http://localhost:4199]
 */
...
async function runMission(cfg) {
  ...
  try {
    ...
    return { id: cfg.id, ok: false, why: 'timeout (480s)', trace };
  } catch (e) {
